...
TOOLS: list[dict] = [
    ...
    },
    {
        ...
        "function": {
            "name": "search_web",
            "description": "搜索互联网获取权威信息。必须调用的场景：① 用户问实时/会变化的事实（新闻/行情/股价/市值/汇率/最新事件）；② 用户提出一个需要核实的断言（如\"长鑫存储市值已超过Intel\"\"某公司上市3周干翻XX\"）——“X是不是真的”这类事实核实必须搜索后回答，禁止仅凭模型内在知识直接下结论；③ 模型不确定或不懂的概念。日常闲聊（问候/吐槽/无事实内容）不需要调用。",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            "name": "earthquake",
            "description": "仅查询地震信息（震级、震中、时间等）。台风、洪水、海啸等其他自然灾害请用 search_web。支持按省份筛选。",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "code": {"type": "string", "description": "Python 代码，用 print() 输出，如 print(f'a={36/3}, b={50-24}')"},
                },
                "required": ["code"],
            },
        },
...
async def _calc(code: str, *args, **kwargs) -> str:
    """执行 Python 表达式并返回结果"""
    import subprocess, tempfile, os
    code = code.strip()
    # 限制危险操作
    BANNED = ("__import__", "open(", "exec(", "eval(", "compile(", "os.", "sys.", "subprocess", "shutil", "import ")
    for b in BANNED:
        if b in code:
            return f"[calc] 不允许的操作: {b}"
    try:
        r = subprocess.run(
            ["python3", "-c", code],
        ...
        if r.stderr:
            ...
            if out:
                ...
                out = err
        return out or "[calc] 无输出"
    except subprocess.TimeoutExpired:
        return "[calc] 超时"
    except FileNotFoundError:
        try:
            r = subprocess.run(["python", "-c", code], capture_output=True, text=True, timeout=5)
            return r.stdout.strip() or r.stderr.strip() or "[calc] 无输出"
        except Exception:
            return "[calc] 执行失败"
...
async def _optimize_search_keywords(query: str) -> str:
    ...
    prompt = f"""把以下搜索词优化为精炼搜索关键词（3-6个词，空格分隔）。
规则：
- 去掉口语词（帮我查/是什么/搜一下/怎么/为什么）
- 展开缩写（5090D→RTX 5090D, 4090→RTX 4090）
- 保留核心名词（公司名/事件/数字），不要堆砌
- **涉及公司市值/股价/行情/上市对比时，保留企业实体名（如"长鑫存储"），
  并追加"股票代码""最新市值"等核实词；若用户问的是某公司市值/是否上市，
  保留该公司名 + 上市主体含义（如"长鑫科技 688825 市值"），避免搜到未上市/过时实体。**
- 只输出关键词，不要解释
...
async def _system_status() -> str:
    ...
    try:
        ...
        from core.config import get_config
        cfg = get_config()
        owner_name = cfg.admin_id_str or cfg.bot_name or "管理员"
        return format_pc_status(owner=owner_name)
    except ImportError:
        return "PC 状态模块未加载"
