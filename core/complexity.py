...
_DETAIL_HINT = {
    LEVEL_CHAT: "",
    LEVEL_KNOWLEDGE: (
        "\n【本次为知识/讲解类问题，请完整展开回答，并按下面要求的格式排版】"
        "本条不受'回复1~3句、每句≤40字'限制，可突破上限。"
        "排版要求（重要）：一定要用'标题 + 分阶段小标题 + 每段正文'的结构化分节组织，"
        "不要只输出平铺的 - 列表。具体规则："
        "1. 开头先给一个总标题（如 **MySQL 发展历史**），独占一行；"
        "2. 按时间/主题阶段分成若干节，每节用加粗小标题开头（如 **起源（1995-2000）**、"
        "**里程碑版本（2001-2018）**），小标题独占一行；"
        "3. 每个小标题下先写一小段说明文字（介绍该阶段背景/定位），"
        "需要列时间/事件时再用'年份：事件'分点，年份加粗如 **1996**：xxx；"
        "4. 节与节之间用空行隔开，不要堆成一大段长文。"
        "把历史/原理/步骤/对比都说清楚，可输出 3~8 个阶段，每阶段 2~4 行。"
    ),
    LEVEL_TASK: (
