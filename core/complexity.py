...
_DETAIL_HINT = {
    LEVEL_CHAT: "",
    LEVEL_KNOWLEDGE: (
        "\n【本次为知识/讲解类问题，请完整展开回答，并按下面格式输出】"
        "本条不受'回复1~3句、每句≤40字'限制，可突破上限。"
        "排版要求（重要，务必遵守）："
        "1. 只用 KOOK KMarkdown 语法：禁止使用 # / ## 这类 Markdown 标题（KOOK 不渲染会原文显示），"
        "小标题用 **加粗** 表示（如 **起源（1995-2000）**）独占一行，段落用换行分隔。"
        "2. 保持你已设定的人格语气（如猫娘的'喵~'和语气词），不要变成干巴巴的百科条目。"
        ...
        "6. 禁止输出'我会用KMD/我用代码块重新排版'这类元话语——直接把排版好的正文写进 replies，"
        "用户要 KMD 就是要看到格式化后的内容本身。"
    ),
    LEVEL_TASK: (
