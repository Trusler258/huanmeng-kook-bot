...
async def main():
    ...
    has_img = any(
        m.get("type") == "section"
        and isinstance(m.get("accessory"), dict)
        and m["accessory"].get("type") == "image"
        for m in card.get("modules", [])
    )
    print(f"OK me → 卡片含右侧头像(accessory): {has_img}")
    ...
    print(f"OK player 25 → 卡片含右侧头像(accessory): {has_img}")
